...
cell_num_plot = metadata_scala %>% filter(cell_count_per_donor > 4) %>% 
  ...
  labs(x = 'Recorded cell number', y = "Microglia score") +
  stat_cor(method = "pearson", label.y = 1, size = 5) +
  theme_classic()  +
...
scala_univar_plots = plot_grid(cell_soma_depth_plot, cell_num_plot, nrow = 2, labels = c('A', 'B'))
...
continous_vars = c("Microglia","age","depth_from_pia_um", "cell_num", "hold_time")
...
# full model
form_full = Microglia ~ biological_sex + scale(age) + scale(depth_from_pia_um) + 
  scale(cell_num) + scale(hold_time) + 
...
res = lapply(c('biological_sex', 'scale(age)', 'scale(depth_from_pia_um)', 
               'scale(cell_num)' , 'scale(hold_time)',
...
```{r}
bottom_row_plot = plot_grid(corresp_plot, scala_mixed_model_plot, nrow = 1, labels = c('D', 'E'))


scala_fig_2_supplement = plot_grid(scala_univar_plots, cell_types_microglia_plot, bottom_row_plot, nrow = 3, rel_heights = c(4, 2.5, 2), labels = c('', 'C', ''))

ggsave(filename = "./output/scala_fig_2_supplement.png", device = "png", width = 12, height = 15, units = "in", dpi = 500, limitsize = F)

ggsave(filename = "./output/scala_fig_2_supplement.svg", device = "svg", width = 12, height = 15, units = "in", dpi = 500, limitsize = F)
```