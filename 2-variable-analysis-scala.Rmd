...
scala_univar_plots = plot_grid(cell_soma_depth_plot, cell_num_plot, hold_time_plot, nrow = 1)
...
# 
# m3 = metadata_scala %>% ggplot(aes(x = cell_num, y = Microglia)) + 
...
continous_vars = c("Microglia","age","depth_from_pia_um", "hold_time", "norm_cell_num")
...
form_full = Microglia ~ biological_sex + scale(age) + scale(depth_from_pia_um) + corresponding_AIT2.3.1_alias + 
  scale(hold_time) + scale(norm_cell_num) + (1|donor_id)
...
res = lapply(c('biological_sex', 'scale(age)', 'scale(depth_from_pia_um)', 'corresponding_AIT2.3.1_alias', 
               'scale(hold_time)', 'scale(norm_cell_num)'), function(V){
...
## stats modeling
```{r}
bottom_row_plot = plot_grid(corresp_plot, scala_mixed_model_plot, nrow = 1)


scala_fig_2_supplement = plot_grid(scala_univar_plots, cell_types_microglia_plot, bottom_row_plot, nrow = 3, rel_heights = c(1.25, 1.5, 1.5))
