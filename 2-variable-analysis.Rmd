...
p2 = metadata_h %>% ggplot(aes(x = reorder(Iba1.bin, Iba1.Cortex), y = Microglia)) + 
  geom_boxplot(outlier.color = "NA", width = 0.6, lwd = 1.0, color = "#114357") + 
  geom_jitter(alpha = 0.5, width = 0.25, shape = 16, size = 1.5, color = "#114357") +
...
svglite("output/figures/univar_human.svg", width = 17, height = 8)
plot_grid(p1, p3, p2, nrow = 1, align = "h", rel_widths = c(1.5,1,1))
dev.off()
...
res$variable = factor(res$variable,
                      levels = c("donor_id","cluster_label","scale(cell_soma_normalized_depth)","medical_conditions","scale(age)","Iba1.bin","biological_sex"),
                      labels = c("Donor","T-type","Normalized depth","Medical condition","Age","Iba1 cortex","Biological sex"))
res$group = "human"
res_h = res
...
svglite("output/figures/univar_mouse.svg", width = 12.15, height = 8)
