...
res = rbind(res_h, res_m, res_mouse_scala)
...
metadata_scala = read_csv('data/patchseq/scala_scMetrics.csv')
metadata_scala = metadata_scala %>% mutate(depth_from_pia_um = as.numeric(`Soma depth (µm)`)) %>%
  dplyr::rename(age = `Mouse age`, Microglia = Macrophage, biological_sex = `Mouse gender`, donor_id = Mouse, 
                hold_time = `Hold Time (min)`, 
                genotype = `Mouse genotype`) %>% 
  separate(Sample, into = c('blah', 'cell_num'), remove = F) %>% mutate(cell_num = as.numeric(cell_num))

metadata_scala_exc = metadata_scala %>% filter(grepl('L(\\d)', top_alm_visp_cluster )) %>% 
...
show_clusters = metadata_scala %>% group_by(corresponding_AIT2.3.1_alias) %>% summarize(cluster_count = n()) 
pal = c("red","gold","green","blue","purple","orange", "black", "grey", "white", "lightgrey")
pal = pal_locuszoom()(10)
scales::show_col(pal)

m1 = metadata_scala %>% ggplot(aes(x = cluster_label, y = Microglia, fill = cluster_label)) + 
  geom_boxplot(outlier.color = "NA", width = 0.6, lwd = 0.8) +
...
m3 = metadata_scala %>% ggplot(aes(x = depth_from_pia_um, y = Microglia)) + 
  geom_point(size = 1.5, shape = 16, color = "black") + 
  geom_smooth(method = "lm", se = FALSE, color = "black", lwd = 1.0) +
  labs(x = "Cell Soma Normalized\nDepth (N = 696)", y = "") +
  theme_classic() +
...
## stats modeling
```{r}
metadata_scala_filt = metadata_scala 
  #dplyr::select(-c("ethnicity","neuron_reconstruction_type","GFAP.ctx.ABC")) %>%
  #mutate(age = as.numeric(gsub(" yrs","",age))) %>% na_if("") %>% na.omit()

continous_vars = c("Microglia","age","depth_from_pia_um", "hold_time", "cell_num")
...
# full model
form_full = Microglia ~ biological_sex + scale(age) + scale(depth_from_pia_um) + corresponding_AIT2.3.1_alias + 
  scale(hold_time) + scale(cell_num) + (1|donor_id)
...
res = lapply(c('biological_sex', 'scale(age)', 'scale(depth_from_pia_um)', 'corresponding_AIT2.3.1_alias', 
               'scale(hold_time)', 'scale(cell_num)'), function(V){
...
res$variable = factor(res$variable,
                      levels = c("donor_id","corresponding_AIT2.3.1_alias","scale(depth_from_pia_um)","scale(age)",
                                 "biological_sex", "scale(hold_time)", "scale(cell_num)"),
                      labels = c("Donor","T-type","Pial depth","Age","Biological sex", "Recording Time", "Cell Num"))
res$group = "mouse_scala"
res_mouse_scala = res

summary(m1)
```
# integrate scala and gouwens microglia effects per cell type
```{r}
metadata_scala = metadata_scala %>% group_by(corresponding_AIT2.3.1_alias) %>% 
  mutate(median_cluster_label_Microglia_scala = mean(Microglia), 
         cell_count = n()) %>% ungroup()

metadata_m = metadata_m %>% group_by(corresponding_AIT2.3.1_alias) %>% 
  mutate(median_cluster_label_Microglia_gouwens = mean(Microglia), cell_count = n()) %>% ungroup()

# metadata_scala$top_alm_visp_subclass = fct_reorder(metadata_scala$top_alm_visp_subclass, metadata_scala$median_cluster_label_Microglia)
...
joined_type_medians %>% ggplot(aes(x = median_cluster_label_Microglia_gouwens, y = median_cluster_label_Microglia_scala, label = corresponding_AIT2.3.1_alias)) + geom_point() +
