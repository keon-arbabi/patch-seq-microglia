...
metadata_h$slice_id = ''
metadata_h$cell_num = 0

for (i in 1:nrow(metadata_h)) {
  split_string = str_split(metadata_h[i, 'cell_specimen_name'], '\\.') %>% unlist
  print(split_string)
  if (length(split_string)== 7){
    slice_num = split_string[5]
    cell_num = split_string[7]
  }else{
    slice_num = split_string[5]
    cell_num = split_string[6]
  }
  print(slice_num)
  print(cell_num)
  metadata_h[i, 'slice_id'] = paste0(metadata_h[i, 'donor_id'], '.', slice_num )
  metadata_h[i, 'slice_num'] = slice_num  %>% as.integer()
  metadata_h[i, 'cell_num'] = cell_num %>% as.integer()
  
}
metadata_h %>% group_by(slice_id) %>% tally()
metadata_h %>% group_by(donor_id) %>% tally()

ephys_h = read_csv('/external/rprshnas01/netdata_kcni/stlab/Public/AIBS_patchseq_2020/human/ephys/processed/ephys_features_berg_w_QC.csv')
metadata_h = left_join(metadata_h, ephys_h, by = "cell_specimen_id")

metadata_h = metadata_h %>% 
  group_by(donor_id) %>% 
  arrange(qc_recording_date) %>%
  mutate(first_cell_time = first(qc_recording_date), 
         rel_recording_time = as.numeric(difftime(qc_recording_date, first_cell_time, units = "hours")))
...
h6 = metadata_h %>% filter(cells_per_donor > 2) %>% ggplot(aes(x = as.factor(slice_num), y = Microglia)) + 
  geom_boxplot(outlier.color = "NA", width = 0.6, lwd =  0.8, fill = "lightgrey") +
  geom_jitter(alpha = 0.3, width = 0.25, shape = 16, size = 1.5) +
  labs(x = "Donor ID", y = "Microglia Score") +
  theme_classic() +
  facet_wrap(~donor_id, scales = "free") +
  theme(
...
h7 = metadata_h %>% filter(cells_per_donor > 2) %>% ggplot(aes(x = qc_recording_date, y = Microglia, color = donor_id)) + 
  #geom_point(outlier.color = "NA", width = 0.6, lwd =  0.8, fill = "lightgrey") +
...
h7 = metadata_h %>% filter(cells_per_donor > 2) %>% ggplot(aes(x = rel_recording_time, y = Microglia)) + 
  #geom_point(outlier.color = "NA", width = 0.6, lwd =  0.8, fill = "lightgrey") +
  geom_point(alpha = 0.3, shape = 16, size = 1.5) + 
  geom_smooth(method = 'lm', se = F, color = "black", lwd = 1.0) + 
  labs(x = "Break-in Time \n(hours after first cell)", y = "Microglia Score") +
  theme_classic() +
  # facet_wrap(~donor_id, scales = "free") +
  theme(
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        ...
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 16)) +
  stat_cor(method = "pearson", label.y = 1, size = 4)

metadata_h$donor_id = fct_reorder(metadata_h$donor_id, -metadata_h$cells_per_donor)

h8 = metadata_h %>% filter(cells_per_donor > 4) %>% ggplot(aes(x = rel_recording_time, y = Microglia, group = donor_id)) + 
  #geom_point(outlier.color = "NA", width = 0.6, lwd =  0.8, fill = "lightgrey") +
  geom_smooth(method = 'lm', se = F, color = "black", lwd = 1.0) + 
  geom_point(alpha = 0.7, shape = 16, size = 1.5) + 
  labs(x = "Break-in Time \n(hours after first cell)", y = "Microglia Score") +
  theme_classic() +
  facet_wrap(~donor_id) +
  #facet_wrap(~donor_id, scales = "free") +
  theme(
        ...
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 16)) +
  stat_cor(method = "pearson", label.y = .9, size = 4)

h9 = metadata_h %>% ggplot(aes(x = qc_seal_gohm, y = Microglia)) + 
  #geom_point(outlier.color = "NA", width = 0.6, lwd =  0.8, fill = "lightgrey") +
  geom_smooth(method = 'lm', se = F, color = "black", lwd = 1.0) + 
  geom_point(alpha = 0.7, shape = 16, size = 1.5) + 
  labs(x = "Initial seal resistance (GOhms)", y = "Microglia Score") +
  theme_classic() +
  # facet_wrap(~donor_id) +
  #facet_wrap(~donor_id, scales = "free") +
  theme(
        ...
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 16)) +
  stat_cor(method = "pearson", label.y = 1.0, size = 4)



human_univar_plots = plot_grid(h5, h4, h3, h7, nrow = 1, align = "h", rel_widths = c(3,1,1, 2))
human_univar_plots
...
continous_vars = c("Microglia","age","depth_from_pia_um", "qc_seal_gohm", "rel_recording_time")
categorical_vars = c("hemisphere","biological_sex","medical_conditions","cluster_label","Iba1.bin","Gfap.bin","donor_id")

metadata_h_filt = metadata_h[, c(continous_vars, categorical_vars)]%>%
...
# full model
form_full = Microglia ~ biological_sex + scale(age) + 
  scale(rel_recording_time) + 
  scale(qc_seal_gohm) + 
  medical_conditions + scale(depth_from_pia_um) + cluster_label + Iba1.bin + Gfap.bin + (1|donor_id)
...
res = lapply(c('biological_sex', 'scale(age)', 'medical_conditions','scale(depth_from_pia_um)', 
               'scale(rel_recording_time)', 
               'scale(qc_seal_gohm)',
               'cluster_label', 'Iba1.bin', "Gfap.bin"), function(V){
...
res$variable = factor(res$variable,
                      levels = c("donor_id","cluster_label","scale(depth_from_pia_um)", "scale(rel_recording_time)",
                                 "scale(qc_seal_gohm)",
                                 "medical_conditions","scale(age)","Iba1.bin","Gfap.bin","biological_sex"),
                      labels = c("Donor","T-type","Pial depth", "Break-in time",
                                 "Seal res.",
                                 "Medical condition","Age","IBA1 protein","GFAP protein","Biological sex"))
res$group = "human"
res_h = res %>% as_tibble()
...
metadata_m = fread(file = here("data","patchseq","20200711_patchseq_metadata_mouse.csv"), data.table = FALSE) %>%
...
metadata_m = metadata_m %>% dplyr::rename(corresponding_AIT2.3.1_alias = `T-type Label`) # do this cause we iniitally wrote the code with the older metadata sheet and fields
...
metadata_m = metadata_m %>% separate(cell_specimen_name, into = c("first", "slice_num_first", "slice_num_second", "cell_num"), remove = F, sep = '\\.') 

metadata_m = metadata_m %>% mutate(slice_name = paste(slice_num_first, slice_num_second, sep = '.'), 
                                   slice_id = paste(donor_id, slice_name, sep = '.'))

metadata_m = metadata_m %>% mutate(cortical_layer = tidyr::extract_numeric(structure)) 

ephys_m = read.csv('/external/rprshnas01/netdata_kcni/stlab/Public/AIBS_patchseq_2020/mouse/ephys/processed/ephys_features_gouwens_w_QC.csv')
metadata_m = metadata_m %>% 
  left_join(., ephys_m, by = "cell_specimen_id")

metadata_m = metadata_m %>% filter(!qc_recording_date == '') %>%
  mutate(qc_recording_date = as.POSIXct(qc_recording_date)) %>%
  group_by(donor_id) %>% 
  arrange(qc_recording_date) %>%
  mutate(first_cell_time = first(qc_recording_date), 
         rel_recording_time = as.numeric(difftime(qc_recording_date, first_cell_time, units = "hours"))) %>% 
  mutate(cortical_layer = (str_extract(structure, "[0-9].?[0-9]?")))
...
metadata_m = metadata_m %>% group_by(donor_id) %>% mutate(median_donor_id_Microglia = median(Microglia), cells_per_donor = n()) %>% ungroup()
metadata_m$donor_id = fct_reorder(as.factor(metadata_m$donor_id), metadata_m$median_donor_id_Microglia)
...
m4 = metadata_m %>% filter(cells_per_donor > 12) %>% ggplot(aes(x = as.factor(slice_name), y = Microglia)) + 
  geom_boxplot(outlier.color = "NA", width = 0.6, lwd = 0.8) +
  geom_jitter(size = 1.5, shape = 16, color = "black") + 
  geom_smooth(method = "lm", se = FALSE, color = "black", lwd = 1.0) +
  theme_classic() +
  facet_wrap(~donor_id, scales = "free_x")

m7 = metadata_m  %>% filter(cells_per_donor > 5) %>% ggplot(aes(x = rel_recording_time, y = Microglia)) + 
  #geom_point(outlier.color = "NA", width = 0.6, lwd =  0.8, fill = "lightgrey") +
  geom_point(alpha = 0.3, shape = 16, size = 1.5) + 
  geom_smooth(method = 'lm', se = F) + 
  # labs(x = "Donor ID", y = "Microglia Score") +
...
m8 = metadata_m %>% filter(!is.na(cortical_layer)) %>% ggplot(aes(x = as.factor(cortical_layer), y = Microglia)) + 
  geom_boxplot(outlier.color = "NA", width = 0.6, lwd = 0.8, alpha = 0.75) +
  geom_jitter(alpha = 0.1, width = 0.25, shape = 16, size = 1.75) +
  labs(y = "Microglia Score", x = "Cortical layer") +
  scale_fill_locuszoom() +
  #scale_fill_manual(values = pal) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
...
m9 = metadata_m %>% filter(!is.na(cortical_layer)) %>% ggplot(aes(x = as.factor(cortical_layer), y = Microglia)) + 
  geom_boxplot(outlier.color = "NA", width = 0.6, lwd = 0.8, alpha = 0.75) +
  geom_jitter(alpha = 0.1, width = 0.25, shape = 16, size = 1.75) +
  labs(y = "Microglia Score", x = "Cortical layer") +
  scale_fill_locuszoom() +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        ...
        axis.text = element_text(size = 14),
        axis.title = element_text(size = 16)) +
  stat_compare_means(method = "kruskal.test", label.y = 0.9, label.x = 1.5, size = 4) +
...
mouse_layer_plot = plot_grid(m8, m9, nrow = 2, align = "hv", rel_widths = c(1, 2), labels = c('A', 'B'))
mouse_layer_plot
ggsave(filename = "./output/mouse_layer_plot.png", device = "png", width = 12, height = 14, units = "in", dpi = 500, limitsize = F)
ggsave(filename = "./output/mouse_layer_plot.svg", device = "svg", width = 12, height = 14, units = "in", dpi = 500, limitsize = F)
...
continous_vars = c("Microglia","age", "rel_recording_time", "qc_seal_gohm")
categorical_vars = c("hemisphere","biological_sex","corresponding_AIT2.3.1_alias","cluster_label", "donor_id")

metadata_m_filt = metadata_m[, c(continous_vars, categorical_vars)] %>%  
...
# full model
form_full = Microglia ~ biological_sex + scale(age) + scale(rel_recording_time) + scale(qc_seal_gohm) +
  corresponding_AIT2.3.1_alias + (1|donor_id)
...
# partial model testing
res = lapply(c('biological_sex', 'scale(age)', 'scale(rel_recording_time)', 'scale(qc_seal_gohm)',
               'corresponding_AIT2.3.1_alias'), function(V){
...
res[,2:7] = sapply(res[, 2:7], as.numeric)
res$variable = factor(res$variable,
                      levels = c("donor_id","corresponding_AIT2.3.1_alias","scale(age)",
                                 "scale(rel_recording_time)", "scale(qc_seal_gohm)",
                                 "biological_sex"),
                      labels = c("Donor","T-type","Age","Break-in time", "Seal res.", "Biological sex"))
res$group = "mouse"
res_m = res %>% as_tibble()
...
plot_grid(h8, h9, 
          nrow = 2, labels = c('A', 'B'), align = "v", axis = "l", rel_heights = c(2,1), scale = 0.9)

ggsave(filename = "./output/microglia_vs_break_in_time_supp.svg", device = "svg", width = 12, height = 14, units = "in", dpi = 500, limitsize = F)
ggsave(filename = "./output/microglia_vs_break_in_time_supp.png", device = "png", width = 12, height = 14, units = "in", dpi = 500, limitsize = F)
