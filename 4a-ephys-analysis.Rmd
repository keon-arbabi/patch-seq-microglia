...
ephys_h = read_csv('/external/rprshnas01/netdata_kcni/stlab/Public/AIBS_patchseq_2020/human/ephys/processed/ephys_features_berg_w_QC.csv')
  # rename(ephys_session_id = cell_specimen_id)
...
metadata_h = fread(file = here("data","patchseq","20200625_patchseq_metadata_human.csv"), data.table = FALSE) %>% 
  ...
  dplyr::select(c(cell_specimen_id, ephys_session_id, sample_id, donor_id, cell_soma_normalized_depth, cluster_label, depth_from_pia_um, cell_specimen_name)) %>%
  left_join(., qcMetrics_h, by = "sample_id") %>%
  left_join(., ephys_h, by = "cell_specimen_id")
...
metadata_h %<>% mutate(
                        ...
                        width = log10(rheo_width * 1000),
                        apamp = rheo_peak_v - rheo_threshold_v, 
                        ahpamp = rheo_threshold_v - rheo_fast_trough_v,
) %>%
  rename(trough_v = rheo_trough_v,
         threshold_v = rheo_threshold_v)
...
metadata_m %>% group_by(corresponding_AIT2.3.1_alias) %>% summarize(corr_val_rmp = cor(Microglia, v_baseline, use = 'pairwise.complete.obs'),
...
effects_df_output = effects_df_output %>% mutate(conf.low = estimate - std.error * 1.96, 
...
bar_graph_plot = effects_df_output  %>% 
  ...
  geom_errorbar(aes(x = ephys_prop, 
                ...
                position = "dodge2") +
  theme_classic() + 
  geom_text(aes(label = ifelse(sig, "*", ""), y = y_asterisk_pos, group = species), color = "black",
...
full_plot = plot_grid(ephys_example_plots, bar_graph_plot, nrow = 2, rel_heights = c(1.3, 1))
full_plot

ggsave(filename = "./output/figure_4a.svg", device = "svg", width = 7, height = 10, units = "in", dpi = 500, limitsize = F)

ggsave(filename = "./output/figure_4a.png", device = "png", width = 7, height = 10, units = "in", dpi = 500, limitsize = F)
