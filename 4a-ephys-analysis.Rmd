...
model_features_list = c('input_resistance', 'rheobase_i', 'width', 'apamp', 'ahpamp', 'threshold_v', 'trough_v', 'v_baseline', 'sag', 'adapt')
...
for(f in model_features_list){
  ...
  tidy_df$ephys_prop = f
  tidy_df$species = 'mouse_gouwens'
  keep_df = bind_rows(keep_df, tidy_df)
  
  formula = paste0('scale(', f, ') ~ 1 + (1|corresponding_AIT2.3.1_alias) + Microglia + (1|donor_id)')
  print(formula)
  tidy_df = tidy(lmer(formula, data = metadata_scala, REML = FALSE)) %>%
    filter(effect == "fixed") %>%
    mutate(term = str_replace(term, "corresponding_AIT2.3.1_alias", "")) %>%
    mutate(term = recode(term, `scale(Microglia)` = "Microglia")) 
  tidy_df$ephys_prop = f
  tidy_df$species = 'mouse_scala'
  keep_df = bind_rows(keep_df, tidy_df)
}
...
bar_graph_plot = keep_df %>% filter(term == 'Microglia', !ephys_prop %in% remove_ephys_props) %>% 
  ...
  theme_classic() + 
  scale_fill_manual(values = c('#9632B8FF', '#EEA236FF', "dark green")) +
...
full_plot = plot_grid(ephys_example_plots, bar_graph_plot, nrow = 2, rel_heights = c(1, 1.3))
full_plot
