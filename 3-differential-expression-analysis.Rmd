...
#ggsave(filename = "./output/figure_1.svg", device = "svg", width = 14, height = 12, units = "in", dpi = 700, limitsize = F)
...
de_up_diff = de_h %>% filter(gene %in% setdiff(de_h_up, de_h_ref_up)) %>% top_n(1000, wt = p_val) %>% pull(gene, avg_log2FC)
de_down_diff = de_h %>% filter(gene %in% setdiff(de_h_down, de_h_ref_down)) %>% top_n(-1000, wt = p_val) %>% pull(gene, avg_log2FC)
...
de_m_up = de_m %>% filter(avg_log2FC > 2.5, p_val < 0.01) %>% arrange(desc(avg_log2FC)) %>% pull(gene, avg_log2FC)
de_m_down = de_m %>% filter(avg_log2FC < -2.5, p_val < 0.01) %>% arrange(desc(avg_log2FC)) %>% pull(gene, avg_log2FC)
de_m_ref_up = de_m_ref %>% filter(avg_log2FC > 2.5, p_val < 0.01) %>% arrange(desc(avg_log2FC))  %>% pull(gene, avg_log2FC)
de_m_ref_down = de_m_ref %>% filter(avg_log2FC < -2.5, p_val < 0.01) %>% arrange(desc(avg_log2FC)) %>% pull(gene, avg_log2FC)

de_up_diff = de_m %>% filter(gene %in% setdiff(de_m_up, de_m_ref_up)) %>% top_n(1000, wt = p_val) %>% pull(gene, avg_log2FC)
de_down_diff = de_m %>% filter(gene %in% setdiff(de_m_down, de_m_ref_down)) %>% top_n(-1000, wt = p_val) %>% pull(gene, avg_log2FC)
...
ggplot(data = de_m, aes(x = avg_log2FC, y = -log10(p_val), col = diffexp, label = delabel)) +
        geom_point() +
        geom_text_repel(max.overlaps = 100, size = 8) +
