...
matrix_h %>%
  ...
  dplyr::select("sample_id", "TNF") %>%
  left_join(., metadata_h, by = "sample_id") %>%
  ggplot(., aes(x = TNF, y = Microglia)) +
  geom_point() + 
...
human_vol_p = ggplot(de_h, aes(x = avg_log2FC, y = -log10(p_val), color = color, label = label)) +
  ...
  geom_text_repel(max.overlaps = 18, show.legend = F, box.padding = 0.25, size = 4) +
  scale_color_manual(values = cols, labels = c("Up", "Up (Patch-seq only)", "Down")) +
  scale_x_continuous(limits = c(-3.0, 10), breaks = seq(-2.5,10,2.5)) +
...
de_m$p_val = ifelse(de_m$p_val == 0.000000e+00, sample(seq(1,9,0.01)*1.0e-299, sum(de_m$p_val == 0.000000e+00, na.rm = T)), de_m$p_val) # deal with zero p-values for plotting 
...
genes_select = c("Tnf","Il1b","Il6","Ccl2")
...
de_m = de_m %>% 
  ...
  mutate(label_pat = case_when(color == "DIFF" & !gene %in% genes_select ~ gene)) %>%
  mutate(label_ref = case_when(color == "UP" & !gene %in% genes_select ~ gene)) %>%
  mutate(label_select = case_when(gene %in% genes_select ~ gene))

cols = c("UP" = "#9632B8FF", "DIFF" = "#D28212", "DOWN" = "#357EBDFF", "NO" = "lightgrey")

mouse_vol_p = ggplot(de_m, aes(x = avg_log2FC, y = -log10(p_val), color = color)) +
  geom_point(shape = 16, alpha = 1) + 
  geom_text_repel(aes(label = label_pat), max.overlaps = 18, show.legend = F, box.padding = 0.25, size = 4.5) +
  geom_text_repel(aes(label = label_ref), max.overlaps = 15, show.legend = F, box.padding = 0.25, size = 4.5) +
  geom_text_repel(aes(label = label_select), max.overlaps = 100, show.legend = F, size = 4.5) +
  scale_color_manual(values = cols, labels = c("Up", "Up (Patch-seq only)", "Down")) +
  scale_x_continuous(limits = c(-3.0, 10.0), breaks = seq(-2.5,10,2.5)) +
