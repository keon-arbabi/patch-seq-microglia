...
unzip(paste0(data_path, 'm1_patchseq_intron_counts.csv.gz'))

meta = read_delim(paste0(data_path, 'm1_patchseq_meta_data.csv'), delim = '\t')
...
scala_exon_counts = read_delim(paste0(data_path, 'm1_patchseq_exon_counts.csv'), delim = ',')
scala_intron_counts = read_delim(gzfile(paste0(data_path, 'm1_patchseq_intron_counts.csv.gz')), delim = ',')
...
# pat_df$contam_type = 'L2.3.IT' # hard-coded
...
mouse_contam_p = ggplot() + 
  ...
  geom_density(data = scala_qc, aes(x = Microglia, group = major_type), 
               size = 1.5, color = "black", fill = "grey", alpha = 0.25) + 
